...
BEGIN
  ...
  ELSIF provider_role = 'food_partner' THEN
    RETURN EXISTS (
...
USING (
  ...
  (get_user_role() = 'nutritionist' AND 
   ...
   is_patient_provider(patient_id, 'nutritionist')) OR
  (get_user_role() = 'doctor' AND 
   is_patient_provider(patient_id, 'doctor'))
);
...
USING (
  nutritionist_id = auth.uid() OR
  patient_id = auth.uid() OR
  (get_user_role() = 'food_partner' AND 
   is_patient_provider(patient_id, 'food_partner'))
);
...
BEGIN
  ...
  ELSIF to_role = 'yoga' THEN
    ...
    WHERE patient_id = patient_id_param 
    LIMIT 1;
  ELSIF to_role = 'food_partner' THEN
