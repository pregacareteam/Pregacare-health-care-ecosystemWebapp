-- ============================================================================
-- PREGACARE COMPREHENSIVE HEALTHCARE ECOSYSTEM - COMPLETE DATABASE SCHEMA
-- ============================================================================
-- Supporting all provider roles with proper RBAC and data relationships
...
CREATE OR REPLACE FUNCTION generate_readable_id(prefix TEXT, table_name TEXT, id_column TEXT)
RETURNS TEXT AS $$
DECLARE
    counter INTEGER;
    new_id TEXT;
BEGIN
    ...
    EXECUTE format('SELECT COALESCE(MAX(CAST(SUBSTRING(%I FROM 2) AS INTEGER)), 0) + 1 FROM %I WHERE %I LIKE $1 || ''%%''', 
                   id_column, table_name, id_column) 
    INTO counter 
    USING prefix;
